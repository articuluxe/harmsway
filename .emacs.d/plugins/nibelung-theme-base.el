...
(defun nibelung-theme-create (theme-name palette)
  ...
  (let* ((bg                  (plist-get palette :bg))
    ...
    (custom-theme-set-faces
     ...
     ;; Transient
     `(transient-key-stay ((t (:foreground ,emphasis))))
     `(transient-key-exit ((t (:foreground ,rainbow-red))))
     `(transient-heading ((t (:foreground ,level4))))
     ...
     `(eglot-highlight-symbol-face ((t (:background ,level0 :underline (:color ,level2 :style line)))))
     ...
     ;; Dape (debugger)
     `(dape-breakpoint-face ((t (:foreground ,rainbow-red))))
     `(dape-source-line-face ((t (:underline (:color ,emphasis :style line :position -1)))))
     `(dape-repl-error-face ((t (:foreground ,rainbow-red))))
     `(dape-exception-description-face ((t (:foreground ,rainbow-orange))))
     `(dape-expression-face ((t (:foreground ,rainbow-cyan))))
     `(dape-hits-face ((t (:foreground ,rainbow-orange))))
     `(dape-log-face ((t (:foreground ,rainbow-green))))
     `(dape-inlay-hint-face ((t (:foreground ,level3))))
     `(dape-header-line-active-face ((t (:background ,level1 :foreground ,emphasis))))
     `(dape-header-line-inactive-face ((t (:background ,level0 :foreground ,level3))))
     `(dape-breakpoint-until-face ((t (:foreground ,rainbow-cyan))))

    ;; Data formats: CSV
    `(csv-separator-face ,optional)

    ;; Data formats: tabulated lists (proced, package menu, ibuffer)
    `(tabulated-list-fake-header ((t (:inherit default :background ,level0 :foreground ,level6))))
    `(buffer-menu-buffer ,symbol)
    `(proced-user ,builtin)
    `(proced-pid ,builtin)
    `(proced-ppid ,builtin)
    `(proced-pgrp ,builtin)
    `(proced-sess ,builtin)
    `(proced-session-leader-pid ,builtin)
    `(proced-emacs-pid ,builtin)
    `(proced-executable ,symbol)
    `(proced-mem ,constant)
    `(proced-cpu ,constant)
    `(proced-time-colon ,optional)
    `(proced-memory-low-usage ,optional)
    `(proced-memory-medium-usage ,constant)
    `(proced-memory-high-usage ((t (:foreground ,rainbow-orange))))
    `(proced-run-status-code ((t (:foreground ,emphasis))))
    `(proced-interruptible-sleep-status-code ,optional)
    `(proced-uninterruptible-sleep-status-code ((t (:foreground ,rainbow-red))))
    `(proced-mark ,bold)
    `(proced-marked ,bold)
    `(proced-sort-header ((t (:foreground ,level6 :underline t))))
    `(package-name ,symbol)
    `(package-description ((t (:foreground ,level3))))
    `(package-help-section-name ,symbol)
    `(package-status-available ,constant)
    `(package-status-avail-obso ,optional)
    `(package-status-built-in ,builtin)
    `(package-status-dependency ,constant)
    `(package-status-disabled ,optional)
    `(package-status-external ,builtin)
    `(package-status-from-source ,builtin)
    `(package-status-held ((t (:foreground ,emphasis))))
    `(package-status-incompat ((t (:foreground ,rainbow-red))))
    `(package-status-installed ,constant)
    `(package-status-new ((t (:foreground ,emphasis))))
    `(package-status-unsigned ((t (:foreground ,rainbow-orange))))
    `(ibuffer-locked-buffer ,constant)
    `(browse-url-button ,link)
    `(icon-button ,link)

    ;; Data formats: EPA
    `(epa-field-name ,constant)
    `(epa-field-body ,text)
    `(epa-string ,optional)
    `(epa-mark ,bold)
    `(epa-validity-high ((t (:foreground ,emphasis))))
    `(epa-validity-medium ,constant)
    `(epa-validity-low ,optional)
    `(epa-validity-disabled ,optional)

    ;; Data formats: XML (nxml)
    `(nxml-text ,text)
    `(nxml-name ,builtin)
    `(nxml-delimited-data ,optional)
    `(nxml-ref ,constant)
    `(nxml-glyph ,constant)
    `(nxml-delimiter ,optional)
    `(nxml-hash ,optional)
    `(nxml-tag-delimiter ,optional)
    `(nxml-tag-slash ,optional)
    `(nxml-element-prefix ,builtin)
    `(nxml-element-colon ,optional)
    `(nxml-element-local-name ,constant)
    `(nxml-attribute-prefix ,builtin)
    `(nxml-attribute-colon ,optional)
    `(nxml-attribute-local-name ,builtin)
    `(nxml-attribute-value ,optional)
    `(nxml-attribute-value-delimiter ,optional)
    `(nxml-namespace-attribute-prefix ,builtin)
    `(nxml-namespace-attribute-colon ,optional)
    `(nxml-namespace-attribute-xmlns ,constant)
    `(nxml-processing-instruction-target ,constant)
    `(nxml-processing-instruction-content ,optional)
    `(nxml-processing-instruction-delimiter ,optional)
    `(nxml-cdata-section-CDATA ,constant)
    `(nxml-cdata-section-content ,text)
    `(nxml-cdata-section-delimiter ,optional)
    `(nxml-char-ref-delimiter ,optional)
    `(nxml-char-ref-number ,constant)
    `(nxml-entity-ref-delimiter ,optional)
    `(nxml-entity-ref-name ,constant)
    `(nxml-markup-declaration-delimiter ,optional)
    `(nxml-prolog-keyword ,constant)
    `(nxml-prolog-literal-content ,optional)
    `(nxml-prolog-literal-delimiter ,optional)
    `(nxml-heading ,symbol)
    `(nxml-outline-ellipsis ,optional)
    `(nxml-outline-active-indicator ,optional)
    `(nxml-outline-indicator ,optional)
    `(rng-error ((t (:underline (:color ,rainbow-red :style wave)))))
    `(sgml-namespace ,builtin)

    ;; Data formats: table.el
    `(table-cell ((t (:inherit default))))

    ;; Data formats: so-long mode line
    `(so-long-mode-line-active ((t (:foreground ,emphasis))))
    `(so-long-mode-line-inactive ((t (:foreground ,level2))))
  ...
  ;; Ensure full font-lock decoration. With `nil' (set e.g. by
  ;; nano-emacs defaults) major modes render at a minimal keyword
  ;; level: code in eglot/corfu doc popups and most keywords, builtins
  ;; and strings in python buffers get no faces at all and look gray.
