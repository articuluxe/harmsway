...
(use-package proviso
  ...
  :bind (("C-c gg" . proviso-grep)
         ...
         ("C-c pf" . proviso-find-file-no-project)
         ("C-c pcr" . proviso-clang-format-buffer-or-region)
         ...
         ("_" . proviso-deploy-add-deploy-term)
...
(defun harmsway-lookup-language-server (lang)
  ...
  (let ((exe (getenv (concat "LSP_SERVER_" (upcase (symbol-name lang))))))
    (if exe
...
(defun harmsway-alt-completion-at-point ()
  ...
  (if (and (fboundp #'eglot-managed-p) (eglot-managed-p))
      (let ((completion-at-point-functions #'eglot-completion-at-point))
...
(use-package syncthing
  ...
  (setq syncthing-default-server-token
        (auth-source-pick-first-password :host "api.syncthing"))
  )
            ...
            (lambda()
              (let ((exe (harmsway-lookup-language-server 'bash)))
                (if (executable-find exe)
                  ...
                  (require 'flymake-collection-bashate)
                  (flymake-mode 1))
...
(add-hook 'shell-mode-hook
          (lambda ()
            ;; set completion
            (setq-local company-alt-backend 'company-shell)
            (define-key shell-mode-map (kbd "TAB") #'company-indent-or-complete-common)
            ))
